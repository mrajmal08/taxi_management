        ...
        <form action="{{ route("admin.drivers.update",[$driver->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('insurance_provider') ? 'has-error' : '' }} col-6">
                ...
                <select class="select2{{ $errors->has('insurance_provider') ? ' is-invalid' : '' }}" name="insurance_provider" id="insurance_provider" >
                        <option @if($driver->insurance_provider == 'none') selected="selected" @endif value="none">None</option>
                        @foreach($insurances as $insurance)
                            <option @if($driver->insurance_provider == $insurance->id) selected="selected" @endif value="{{$insurance->id}}">{{$insurance->name}}</option>
                        @endforeach
                        <option @if($driver->insurance_provider == 'other') selected="selected" @endif value="other">Other</option>
                    </select>
